...
def basic(target: str, genes: list[str], debug: bool = True) -> tuple[int, int, str]:
    ...
    for _ in range(N_POPULATION):
        population.append("".join([random.choice(genes)
    ...
    while True:
        ...
        def evaluate(item: str, main_target: str = target) -> tuple[str, float]:
            ...
            """
            score = len(
                [g for position, g in enumerate(
                    item) if g == main_target[position]]
            )
        ...
        population_score = sorted(
            population_score, key=lambda x: x[1], reverse=True)
        ...
        def mutate(child: str) -> str:
            ...
            if random.uniform(0, 1) < MUTATION_PROBABILITY:
                child_list[random.randint(
                    0, len(child)) - 1] = random.choice(genes)
